...
<template>
    <div class="flex items-center justify-between mt-6">
        ...
        <div class="items-center hidden lg:flex gap-x-3">
            <a v-for="(link, index) in pageLinks" :key="link.label" @click.prevent="goToPage(link.url)" class="px-2 py-1 text-sm text-blue-500 rounded-md bg-blue-100">{{ link.label }}</a>
        </div>
