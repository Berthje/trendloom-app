...
export default {
    ...
    methods: {
        updateSorting($event) {
            this.$emit('update:filterOptions', { ...this.filterOptions, sorting: $event.target.value });
        },
        updateItemCount($event) {
            this.$emit('update:filterOptions', { ...this.filterOptions, itemCount: $event.target.value });
        }
    },
}
</script>
